...
def load_companies(conn: sqlite3.Connection) -> int:
    ...
    if os.path.exists(stage2_path):
        ...
        for _, row in df2.iterrows():
            ...
            # sic_codes can be pipe-separated (e.g. "62012|70229") —
            # take the second one if present, for sic_code_2
            sic_list = str(row.get("sic_codes", "")).split("|")
            sic_code_1 = sic_list[0].strip() if sic_list else ""
            sic_code_2 = sic_list[1].strip() if len(sic_list) > 1 else ""
            ...
            cursor.execute("""
                INSERT OR REPLACE INTO companies (
                    ...
                    sic_code_1, sic_code_2, incorporation_date,
                    company_age_years,
            ...
            """, (
                ...
                postcode,
                postcode_district,
                str(row.get("current_address", "")).strip(),
                str(row.get("current_town", "")).strip(),
                sic_code_1,
                sic_code_2,
                creation_date if creation_date != "nan" else "",
                age_years,
...
def _safe_float(val) -> float:
    ...
    try:
        ...
        return None
def _plausible_current_ratio(row) -> float | None:
    """Guard against the mis-tagged current_liabilities defect (Section 6.3a):
    exclude current_ratio when current_liabilities is implausibly near zero
    against substantial current_assets."""
    ratio = _safe_float(row.get("current_ratio"))
    assets = _safe_float(row.get("current_assets"))
    liabilities = _safe_float(row.get("current_liabilities"))
    if liabilities is not None and liabilities < 100 and assets is not None and assets > 10000:
        return None  # flagged implausible — exclude from storage/scoring
    return ratio
